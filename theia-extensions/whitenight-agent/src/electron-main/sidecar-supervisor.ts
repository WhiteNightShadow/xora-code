...
import { deepRedact } from './session-repository';
...
export class GrokSidecarSupervisor {
    ...
    protected resolveBinary(): string {
        const override = process.env.WHITENIGHT_GROK_BINARY;
    ...
    protected assertBinary(candidate: string, allowedRoot: string | undefined): string {
        ...
        if (process.platform !== 'win32' && (fs.statSync(real).mode & 0o111) === 0) {
    ...
    protected flushStderr(): void {
        this.stderrCarry += this.stderrDecoder.end();
        if (this.stderrCarry) {
